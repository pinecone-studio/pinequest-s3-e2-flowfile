...
type LegacySub = Submission & { status?: string }
type LegacyUser = User & { studentId?: string }

export function GradeStudentSidebar({ exam, students, submissions, selectedStudentId, gradedCount, needsGradingCount, onSelect }: {
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-2">
          {(students as LegacyUser[]).map(student => {
            const submission = (submissions as LegacySub[]).find(s => s.studentId === student.id)
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-text-secondary">{student.studentId}</div>
                  </div>
                  {isGraded ? (
