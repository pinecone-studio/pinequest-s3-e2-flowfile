...
export function QuestionTabNew({ questionText, questionType, questionOptions, correctAnswer, questionPoints, matchingPairs, onQuestionText, onQuestionType, onQuestionOptions, onCorrectAnswer, onQuestionPoints, onMatchingPairs, onAddQuestion, onCancel, actionLabel = 'Нэмэх' }: {
  ...
  onCancel?: () => void; actionLabel?: string
}) {
  const selectedTypeMeta = questionTypeMeta[questionType]
  const SelectedTypeIcon = selectedTypeMeta.icon
  const actionIcon = actionLabel === 'Нэмэх' ? <Plus size={14} strokeWidth={1.5} /> : <Check size={14} strokeWidth={1.5} />
  ...
  return (
    <div className="bg-white rounded-lg border border-card-border p-6 space-y-4">
            ...
            {questionOptions.map((opt, index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                <input type="text" value={opt} onChange={(e) => {
                  ...
                  newOpts[index] = nextValue
                  onQuestionOptions(newOpts)
                  if (questionType === 'single' && correctAnswer === opt) onCorrectAnswer(nextValue)
                  if (questionType === 'multiple' && Array.isArray(correctAnswer) && correctAnswer.includes(opt)) {
                    onCorrectAnswer(correctAnswer.map(answer => answer === opt ? nextValue : answer))
                  }
                }} placeholder={`Хариулт ${String.fromCharCode(65 + index)}`} className="flex-1 px-3 py-2 border border-input-border rounded-lg text-[14px] focus:border-primary focus:ring-1 focus:ring-primary/20 focus:outline-none transition-colors" />
                {questionOptions.length > 2 && (<button type="button" onClick={() => {
                  onQuestionOptions(questionOptions.filter((_, i) => i !== index))
                  if (questionType === 'single' && correctAnswer === opt) onCorrectAnswer('')
                  if (questionType === 'multiple' && Array.isArray(correctAnswer) && correctAnswer.includes(opt)) {
                    onCorrectAnswer(correctAnswer.filter(answer => answer !== opt))
                  }
                }} className="p-1.5 text-text-secondary hover:text-red-accent"><Trash2 size={14} strokeWidth={1.5} /></button>)}
              </div>
            ))}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-divider">
        ...
        <div className="flex items-center gap-2">
          {onCancel && (
            <button type="button" onClick={onCancel} className="px-4 py-2 border border-card-border rounded-lg text-[13px] font-medium text-foreground hover:bg-table-header transition-colors">
              Болих
            </button>
          )}
          <button type="button" onClick={onAddQuestion} disabled={!questionText.trim()} className="px-4 py-2 bg-primary text-white rounded-lg text-[13px] font-medium hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center gap-1.5">{actionIcon}{actionLabel}</button>
        </div>
      </div>
    </div>
  )
